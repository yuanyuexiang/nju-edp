...
Run: python3 assets/video/edp-brand-demo-v1/build_video.py
"""
...
SCENES = [
    (0, 5, "变化，正在重写经营的答案。", ["变化，正在重写经营的答案。"]),
    (5, 11, "面对新的市场，如何看清方向，做出判断？", ["面对新的市场", "如何看清方向，做出判断？"]),
    (11, 17, "走进南京大学，E D P。让学习，回应真实的管理问题。", ["走进南京大学 EDP", "让学习，回应真实的管理问题"]),
    (17, 23, "探索个人提升与企业定制，开启下一阶段的学习。", ["探索个人提升与企业定制", "开启下一阶段的学习"]),
    (23, 30, "南京大学，E D P。了解项目详情，请通过官方渠道咨询。", ["南京大学 EDP", "了解项目详情，请通过官方渠道咨询"]),
]
...
def scene_layer(i, local):
    ...
    y = lambda v: v + rise
    tags = ["01 / A NEW PERSPECTIVE", "02 / BETTER QUESTIONS", "03 / NANJING UNIVERSITY",
    ...
    if i == 0:
        txt(d, (83, y(385)), "世界在变", 119, WHITE, True)
        txt(d, (83, y(548)), "判断，向前", 119, GOLD, True)
        txt(d, (93, y(760)), "下一阶段", 37, MUTED)
        txt(d, (93, y(822)), "企业需要怎样的管理者？", 43)
        txt(d, (94, 1395), "从一个真实的经营问题开始。", 34, MUTED)
    elif i == 1:
        txt(d, (85, y(378)), "面向新的市场", 96, WHITE, True)
        txt(d, (85, y(514)), "重新提出问题", 96, GOLD, True)
        for k, (a, b) in enumerate([("01", "方向，如何看清？"), ("02", "决策，如何做出？"),
                                     ("03", "组织，如何向前？")]):
        ...
        txt(d, (94, y(1000)), "让学习，回应真实的管理问题。", 36, MUTED)
        txt(d, (95, 1398), "EXECUTIVE DEVELOPMENT PROGRAMS", 26, GOLD, latin=True)
    elif i == 3:
        txt(d, (85, y(378)), "让学习", 107, WHITE, True)
        txt(d, (85, y(526)), "回应真实问题", 100, GOLD, True)
        for k, (title, sub, en) in enumerate([
            ("个人提升", "探索下一阶段的学习", "FOR INDIVIDUALS"),
        ...
        txt(d, (85, y(706)), "从一个问题开始", 92, GOLD, True)
...
def sound():
    ...
    for i,(start,end,line,_) in enumerate(SCENES):
        ...
        narration_meta.append({"scene":i+1,"start":start+.6,"duration":len(voice)/sr,"text":line})
...
def main():
    ...
    audio=sound()
    output=ROOT/"南京大学EDP-招生创意样片-30秒.mp4"
